/*
 * Primfaktorzerlegung einer Zahl (Variante 2)
...
int main(int argc, char **argv) {
	...
	while (n > 1) {
		...
		if (n % divisor == 0) {
			...
			// Fange hier den Fall ab, falls "n" nach der Division
			// den Wert 1 bekommen hat und deswegen die Schleife im
			// nächsten Ablauf abbricht, sodass der letzte Primfaktor
			...
			// keine Restteilung mit aktuellem Divisor (mehr) möglich
			// ==> Divisor erhöhen
			divisor++;
		}
	}
